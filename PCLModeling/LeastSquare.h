...
public:
    double a, b;
    LeastSquare(){};
    void fitting(const vector<float>& x, const vector<float>& y)
    {
    ...
    void getYVector(const vector<float>& x, vector<float>& y) const
    {
        int x_size = x.size();
        y.resize(x_size);
    ...
    double getY(const float x) const
    {
        return a*x + b;
    }
};
...
public:
    ...
    ///  
    std::vector<double> &getYs(){ return fitedYs; };
