...
int GetHistogram(PointCloudT &cloud, double& existProportion, double *(&hist), PointT minPt, PointT maxPt, double gridHeight)
{
    ...
    existProportion = existSize / Histogramdim;
...
double Otsu(PointCloudT &cloud, PointT minPt, PointT maxPt, double gridHeight, double& existProportion)
{
    if (gridHeight == 0)
        return 0;

    double *hist = nullptr;
    int Histogramdim = GetHistogram(cloud, existProportion, hist, minPt, maxPt, gridHeight);
...
void setColors(PointCloudT &cloud, uchar r = 255, uchar g = 0, uchar b = 0)
{
...
{
    //ExtractGroundOnElevationHistogram();
    QGround::Fun(cloud_, *ground_);
    QLiqundian::Fun(cloud_);
...
bool QPowerLineClassification::IsHaveLiQunDian(PointCloudT::Ptr cloud)
{
    ...
    *tempCloud = *cloud;
    ...
    sor.setInputCloud(tempCloud);                           //设置待滤波的点云
    sor.setMeanK(10);                               //设置在进行统计时考虑查询点临近点数
    sor.setStddevMulThresh(10);                      //设置判断是否为离群点的阀值
    sor.filter(*tempCloud);                    //存储
    dd("cloudsize=%d,tempcloudsize=%d", cloud->size(), tempCloud->size());
    return !(cloud->size() == tempCloud->size());
}
...
//         else if (IsHaveLiQunDian(curCloud))
//         {
//             dd("检测到离群点过滤");
//             filterVec[i] = curCloud;
//         }
...
#pragma omp parallel for
    for (int i = 0; i < clusters.size(); ++i)
    {
        ...
        dd("密度聚类的点数%d，范围搜索的点数%d，obb扩展点数%d，obb中心点坐标%f,%f,%f，第一点的坐标%f,%f,%f", 
            ptct, radiuIndices.size(), 
            towersIndies[i]->indices.size(), 
            obb.pos.x, obb.pos.y, obb.pos.z, 
            cloud_->at(clusters[i][0]).x, 
            cloud_->at(clusters[i][0]).y, 
            cloud_->at(clusters[i][0]).z);
    }
...
bool QPowerLineClassification::IsLowerClass(PointCloudT &cloud, pcl::KdTreeFLANN<PointT> &ground_kdtree, double groundLower)
{
    ...
    if (ground_kdtree.radiusSearch(max, groundLower, indices, sqr_distances))
    {
        //dd("最高点与地面的间距%f<%f，则删除", sqrt(sqr_distances[0]), groundLower);
        return true;
    }
...
//     //canny边缘检测  
//     liter = threshold;
//     cvCanny(g_pSrcImage, g_pCannyImg, liter, biger, 3);
//     cvShowImage(pstrWindowsCannyTitle, g_pCannyImg);
// }
// 
// void on_trackbar1(int threshold)
// {
//     //canny边缘检测  
//     biger = threshold;
//     cvCanny(g_pSrcImage, g_pCannyImg, liter, biger, 3);
...
// }
// 
// void huadongCanny(const char *path)
// {
//     const char *pstrImageName = path;
//     const char *pstrWindowsSrcTitle = "原图(http://blog.csdn.net/MoreWindows)";
//     const char *pstrWindowsToolBar = "Threshold";
//     const char *pstrWindowsToolBar1 = "Threshold1";
//     dd(pstrImageName);
//     dd("1");
...
//     cvCreateTrackbar(pstrWindowsToolBar, pstrWindowsCannyTitle, &nThresholdEdge, 1000, on_trackbar);
//     int nThresholdEdge1 = 1;
//     cvCreateTrackbar(pstrWindowsToolBar1, pstrWindowsCannyTitle, &nThresholdEdge1, 1000, on_trackbar1);
// 
//     dd("4");
//     //在指定窗口中显示图像  
...
int g_canny_min = 180;
int g_canny_max = 540;
double g_houghLines_rho = 10;
int g_houghLines_threshold = 86;
int g_houghLines_minlen = 100;
int g_houghLines_gap = 30;
const char *houghtitle = "Hough";

 void HoughLines()
 {
//      string temppath = (QDir::tempPath() + "/" + QString::number(getpid()) + "_" + QString::number((int)GetCurrentThread()) + "Gray_Image.jpg").toLocal8Bit().data();
//      IplImage *src = cvLoadImage(temppath.c_str(), 0);
//      if (src)
//      {
...
//          CvSeq *lines = 0;
//          int i;
//          cvCanny(src, dst, g_canny_min, g_canny_max, 3);
//          cvCvtColor(dst, color_dst, CV_GRAY2BGR);
// 
//          // threshold 阈值参数。如果相应的累计值大于 threshold， 则函数返回的这个线段.
//          // param1对概率 Hough 变换，它是最小线段长度.
//          // param2对概率 Hough 变换，这个参数表示在同一条直线上进行碎线段连接的最大间隔值(gap), 即当同一条直线上的两条碎线段之间的间隔小于param2时，将其合二为一。
//          lines = cvHoughLines2(dst, storage, CV_HOUGH_PROBABILISTIC, g_houghLines_rho, CV_PI / 180, g_houghLines_threshold, g_houghLines_minlen, g_houghLines_gap);
...
//          }
// 
//          cvShowImage(houghtitle, color_dst);
// 
//          cvReleaseImage(&src);
//          cvReleaseImage(&dst);
...
//      }
 ...
 void on_trackbar_canny_min(int canny_min)
 {
     g_canny_min = canny_min;
     HoughLines();
 }
 void on_trackbar_canny_max(int canny_max)
 {
     g_canny_max = canny_max;
     HoughLines();
 }
 void on_trackbar_houghLines_rho(int houghLines_rho)
 {
     if (0 == g_houghLines_rho)
         g_houghLines_rho = 1;
     g_houghLines_rho = houghLines_rho;
     g_houghLines_rho /= 10;
     HoughLines();
 }
 void on_trackbar_houghLines_threshold(int houghLines_threshold)
 {
     g_houghLines_threshold = houghLines_threshold;
     HoughLines();
 }
 void on_trackbar_houghLines_minlen(int houghLines_minlen)
 {
     g_houghLines_minlen = houghLines_minlen;
     HoughLines();
 }
 void on_trackbar_houghLines_gap(int houghLines_gap)
 {
     g_houghLines_gap = houghLines_gap;
     HoughLines();
 }


// 用opencv提取到二维直线，然后在点云中定位这条直线投影到的xy平面的所有点。  找到高程连续的线就是电力线
void QPowerLineClassification::UseOpencvFineLine()
{
//     PointCloudT::Ptr tempCloud(new PointCloudT);
//     pcl::copyPointCloud(*cloud_, *tempCloud);
// 
//     // 投影到2维
//     ProjectionXyPlane::Fun(tempCloud);
// 
//     // 把cloud放到二维数组里
//     // 以0.5米分辨率对应像素点
//     // 记录了像素偏差， 到时候恢复到点云里可以根据偏差
// 
//     int width = 0;
//     int height = 0;
//     map<int, map<int, pair<float, float>>> map_pic;   //pair<float, float>记录偏移
//     for (int i = 0; i < tempCloud->size(); ++i)
//     {
//         PointT &pt = tempCloud->at(i);
//         int row = (pt.x - info_.min.x) / 0.5;
//         if (row > width)
//             width = row;
//         int col = (pt.y - info_.min.y) / 0.5;
//         if (col > height)
//             height = col;
//         float xoff = (pt.x - info_.min.x) / 0.5 - (int)((pt.x - info_.min.x) / 0.5);
//         float yoff = (pt.y - info_.min.y) / 0.5 - (int)((pt.y - info_.min.y) / 0.5);
//         map_pic[row][col] = std::make_pair(xoff, yoff);
//     }
//     ++width;
//     ++height;
//     Mat img(width, height, CV_8UC3, Scalar(255,255,255));   // 构造函数的一种
// 
//     //遍历像素点  
//     for (map<int, map<int, pair<float, float>>>::iterator it = map_pic.begin(); it != map_pic.end(); ++it)
//     {
//         int row = it->first;
//         for (map<int, pair<float, float>>::iterator itt = it->second.begin(); itt != it->second.end(); ++itt)
//         {
//             int col = itt->first;
...
// 
//     int canny_min = g_canny_min;
//     int canny_max = g_canny_max;
//     int houghLines_rho = 10;
//     int houghLines_threshold = g_houghLines_threshold;
//     int houghLines_minlen = g_houghLines_minlen;
//     int houghLines_gap = g_houghLines_gap;
//     cvNamedWindow(houghtitle, CV_WINDOW_NORMAL);
//     cvResizeWindow(houghtitle, 1024, 768);
//     //创建滑动条  
//     cvCreateTrackbar("canny_min", houghtitle, &canny_min, 1000, on_trackbar_canny_min);
//     cvCreateTrackbar("canny_max", houghtitle, &canny_max, 1000, on_trackbar_canny_max);
//     cvCreateTrackbar("houg_rho*10", houghtitle, &houghLines_rho, 1000, on_trackbar_houghLines_rho);
//     cvCreateTrackbar("houg_threshold", houghtitle, &houghLines_threshold, 1000, on_trackbar_houghLines_threshold);
//     cvCreateTrackbar("houg_minlen", houghtitle, &houghLines_minlen, 1000, on_trackbar_houghLines_minlen);
//     cvCreateTrackbar("houg_gap", houghtitle, &houghLines_gap, 1000, on_trackbar_houghLines_gap);
//     on_trackbar_houghLines_rho(houghLines_rho);
//     cvWaitKey(0);
// 
//     cvDestroyAllWindows();
}

void QPowerLineClassification::ExtractGroundOnElevationHistogram()
{
    PointCloudT::Ptr cloud = /*PCManage::ins().*/cloud_;

    double planeGridSize =5;
    double gridheight = 0.5;
//     double planeGridSize =/* 0.008f*/atof(ui.lineEdit->text().toLocal8Bit().data());
//     int heightCount = /*0.1f*/atoi(ui.lineEdit_2->text().toLocal8Bit().data());
    ...
    std::vector<std::vector<std::vector<int >> > planeClassify;
    planeClassify.clear();
    planeClassify.resize(3);
    std::vector<std::vector<std::vector<int>>> planeGrids; // 平面网格
    PointT minPt, maxPt;
    pcl::getMinMax3D(*cloud, minPt, maxPt);

    int xGridCount = (maxPt.x - minPt.x) / planeGridSize + 1;
    int yGridCount = (maxPt.y - minPt.y) / planeGridSize + 1;


    // 初始化网格
    planeGrids.resize(xGridCount);
    for (int i = 0; i < xGridCount; ++i)
    {
        planeGrids[i].resize(yGridCount);
    }
    ...
    {
        PointT &pt = cloud->at(i);
        planeGrids[(int)(pt.x - minPt.x) / planeGridSize][(int)(pt.y - minPt.y) / planeGridSize].push_back(i);
    }
    for (int i = 0; i < xGridCount; ++i)
    {
        for (int j = 0; j < yGridCount; ++j)
        {
            std::vector<int> &indices = planeGrids[i][j];
            if (!indices.size() || indices.size() == 1)
            {
                planeClassify[0].push_back(indices);
                setColors(*cloud, indices);
                continue;
            }

            PointCloudT tempCloud;
            pcl::copyPointCloud(*cloud, indices, tempCloud);

            // 如果高程小于5，直接认为是地面。
            PointT minPt, maxPt;
            pcl::getMinMax3D(tempCloud, minPt, maxPt);
            if (maxPt.z - minPt.z < 5)
            {
                planeClassify[0].push_back(indices);
                continue;
            }

            double existProportion = 0;
            double threshold = Otsu(tempCloud, minPt, maxPt, gridheight, existProportion);
            if (threshold == 0)
            {
                // 是点数非常少的一些数据,这种数据不要了
                //planeClassify[0].push_back(indices);
            ...
            if (existProportion > 0.8 && maxPt.z - minPt.z <8)
            {
                planeClassify[0].push_back(indices);
                continue;
            }
            double groundThreshold = threshold + minPt.z;
            std::vector<int> ground;
            std::vector<int> noground;
            for (int k = 0; k < indices.size(); ++k)
            {
                auto & pt = cloud->at(indices[k]);
                if (pt.z < groundThreshold)
                {
                    ground.push_back(indices[k]);
                }
            }
            if (ground.size())
            {
                planeClassify[0].push_back(ground);
            }
        }
    }

    std::vector<std::vector<int>> &classify = planeClassify[0];
    std::vector<int> indices;
    for (int j = 0; j < classify.size(); ++j)
    {
        indices.insert(indices.end(), classify[j].begin(), classify[j].end());
    }

    pcl::PointIndicesPtr groundindices(new pcl::PointIndices);
    groundindices->indices = indices;
    pcl::ExtractIndices<PointT> extract;
    extract.setInputCloud(cloud);
    extract.setIndices(groundindices);
    extract.filter(*ground_);
    // 提取非地面点
    extract.setNegative(true);
...
bool QPowerLineClassification::LikePowerLine(LineInfo &line)
{
    ...
    if (line.sta.Distance(line.end) < 5)
    {
        return false;
    }
...
#pragma omp parallel for
    for (int i = 0; i < jlclassCount; ++i)
    {
        ...
        *indices = juClusters[i];
        ...
        {
            dd("classsize=%d", indices->indices.size());
            continue;
        }
        pcl::ExtractIndices<PointT> extract;
        extract.setInputCloud(cloud_);
...
void QPowerLineClassification::LabelCrashLine(PointCloudT::Ptr crashPoint, double K, double gap)
{
    std::vector<pcl::PointIndices> cluster_indices;
    QOushifenge::Fun(crashPoint, cluster_indices, gap);
...
void QPowerLineClassification::TooNearCheck2()
{
    uint cur, sta;
    sta = GetTickCount();
    ...
    PointCloudT::Ptr allCrashPoint(new PointCloudT);
...
#pragma omp parallel for
    for (int i = 0; i < lines_.size(); ++i)
    {
        ...
        // 得到此根线的最大相交范围点
        PointCloudT::Ptr crashPoint(new PointCloudT); 
        for (int j = 0; j < serachLine->size(); ++j)
        {
            serachNum = groundkdtree.radiusSearch(serachLine->at(j), K, groundindices, groundsqr_distances) + objectkdtree.radiusSearch(serachLine->at(j), K, objectindices, objectsqr_distances);
            if (serachNum)
...
#pragma omp critical 
            for (std::set<int>::iterator it = line_objectindices.begin(); it != line_objectindices.end(); ++it)
            {
                ...
                PointT &pt = groundObjects_[session]->at(*it - sessionnum); 
                {
                    pt.r = 0;
                    pt.g = 255;
                    pt.b = 0;
...
void QPowerLineClassification::AddVtkLines(const boost::shared_ptr<LineInfo> &line)
{
    std::vector<vec>& pts = line->pts;
    bool xsign = (line->sta.x - line->end.x) < 0;
    bool ysign = (line->sta.y - line->end.y) < 0;
    ...
    for (int i = 0; i < pts_size; ++i)
    {
        if (i != 0)
        {
            if (prex - pts[i].x < 0 == xsign && prey-pts[i].y < 0 ==ysign)
            {
                points->InsertNextPoint(pts.at(i).x, pts.at(i).y, pts.at(i).z);
                dd("%d:%f,%f,%f", tick, pts.at(i).x, pts.at(i).y, pts.at(i).z);
                prex = pts.at(i).x;
                prey = pts.at(i).y;
            }
        }
    ...
    spline->ClosedOff();
    vtkSmartPointer<vtkParametricFunctionSource>splineSource(vtkParametricFunctionSource::New());
    splineSource->SetParametricFunction(spline);
    
    vtkSmartPointer<vtkPolyDataMapper>splineMapper(vtkPolyDataMapper::New());
    splineMapper->SetInputConnection(splineSource->GetOutputPort());

    vtkSmartPointer<vtkActor>splineActor(vtkActor::New());
    ...
    line_models_.insert(splineActor);
}

void QPowerLineClassification::AddTube(const boost::shared_ptr<LineInfo> &line, const float radiu)
{
    std::vector<vec>& pts = line->pts;
    bool xsign = (line->sta.x - line->end.x) < 0;
    bool ysign = (line->sta.y - line->end.y) < 0;
    ...
    for (int i = 0; i < pts_size; ++i)
    {
        if (i != 0)
        {
            if (prex - pts[i].x < 0 == xsign && prey - pts[i].y < 0 == ysign)
            {
                vtkIdType pid[1];
                points->InsertNextPoint(pts.at(i).x, pts.at(i).y, pts.at(i).z);
                polyline->GetPointIds()->InsertNextId(/*step, */pid[0]);
                prex = pts.at(i).x;
                prey = pts.at(i).y;
                step++;
                dd("3");
        ...
        {
            vtkIdType pid[1];
            pid[0] = points->InsertNextPoint(pts.at(i).x, pts.at(i).y, pts.at(i).z);
            polyline->GetPointIds()->InsertNextId(/*step, */pid[0]);
            step++;
            dd("2");
        }
    }
    const int pointNum = step;
    pts_size = step;
...
void QPowerLineClassification::AddTube(const std::vector<vec> &pts, const float radiu)
{
    ...
    color_->SetNumberOfTuples(pts_size);
    for (int ii = 0; ii < pts_size; ++ii)
    {
        color_->InsertTuple3(ii, 0, 0, 255);
    }
    tubePolyData->GetPointData()->AddArray(color_);
    ...
    tubeMapper->SetScalarModeToUsePointFieldData();//使用FieldData为对象上色
    ...
    vtkSmartPointer<vtkActor> tubeActor = vtkSmartPointer<vtkActor>::New();
    tubeActor->SetMapper(tubeMapper);
    tubeActor->GetProperty()->SetOpacity(0.1);
...
void QPowerLineClassification::RemoveViewBalls()
{
    ...
    ui.widget->viewer_->updateText("", 0, 0, "crashText");
}
...
void QPowerLineClassification::SetShowClassNosyns(QString classname, bool checked/* = true*/)
{
    ...
    else if (QStringLiteral("碰撞球") == classname)
    {
        ...
        if (checked)
        {
            ...
            double dy = vp[3] - vp[1];
            if(!ui.widget->viewer_->addText(fmt.toUtf8().data(), lineHeight, max(0, (int)dy - (3 + (int)balls_.size()) * lineHeight), 0, 0, 1, "crashText"))
                ui.widget->viewer_->updateText(fmt.toUtf8().data(), lineHeight, max(0, (int)dy - (3 + (int)balls_.size()) * lineHeight), 0, 0, 1, "crashText");
        }
    }
}
...
void QPowerLineClassification::Simplify()
{
